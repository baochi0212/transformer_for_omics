...
class decoderr(nn.Module):
    def __init__(self, key_size, query_size, value_size,
        ...
        self.position = nn.Parameter(torch.rand([num_hiddens]))
        ...
        for i in range(num_layers):
            self.blks.add_module(
                ...
                DecoderBlock(key_size, query_size, value_size, num_hiddens,
                             ...
                             num_heads, dropout, i))
        self.dense = nn.Linear(num_hiddens, key_size)
...
if __name__ == '__main__':
    out = torch.rand([1,100,24])
    x = torch.rand([1,100,24])
    decoder = decoderr(24,24,24,24,[100,24],24,48,8,10,0.5)
    state = decoder.init_state(out, None)
    print(decoder(x,state)[0].shape)